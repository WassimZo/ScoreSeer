...
export default function BaseModal({ children, passwd }) {
  const navigate = useNavigate();

  return (
    <div
      onClick={(e) => !passwd && navigate("/")}
      className="fixed inset-0 bg-slate-700/75 flex justify-center items-center z-10 "
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="relative z-20 bg-slate-50 text-slate-900 h-screen w-full dark:bg-slate-700 lg:w-[40%] lg:h-[80%] lg:rounded-lg"
      >
        {!passwd && (
